#!/bin/bash
# Automated setup script for Slurm Heartbeat on Vega (Slovenia)
set -e

echo "=== Slurm Heartbeat Setup for Vega ==="
...
if [[ -f "examples/vega/config.yaml" ]]; then
...
if [[ ! -f "$HOME/.slurm/heartbeat/server.crt" ]]; then
    ...
    openssl req -x509 -new -nodes -key "$HOME/.slurm/heartbeat/ca.key" \
        ...
        -subj "/C=SI/O=VEGA/OU=Vega/CN=vega-si"
