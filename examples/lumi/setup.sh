#!/bin/bash
# Automated setup script for Slurm Heartbeat on LUMI (CSC Finland)
# Run this with sudo for production deployment

set -e

echo "=== Slurm Heartbeat Setup for LUMI ==="
echo ""
...
if [[ -f "examples/lumi/config.yaml" ]]; then
    cp examples/lumi/config.yaml /etc/slurm/heartbeat/config.yaml
    echo "✓ Copied config.yaml to /etc/slurm/heartbeat/"
else
    echo "Warning: examples/lumi/config.yaml not found"
...
if [[ ! -f "/etc/slurm/heartbeat/server.crt" ]] || [[ ! -f "/etc/slurm/heartbeat/server.key" ]]; then
    ...
    openssl req -x509 -new -nodes -key /etc/slurm/heartbeat/ca.key \
        ...
        -subj "/C=FI/O=CSC/OU=LUMI/CN=lumi-csc-fi"
    ...
    echo "  Note: For production, use CSC PKI or EFP CA certificates"
else
