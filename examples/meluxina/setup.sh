#!/bin/bash
# Automated setup script for Slurm Heartbeat on MeluXina (Luxembourg)
set -e

echo "=== Slurm Heartbeat Setup for MeluXina ==="
...
if [[ -f "examples/meluxina/config.yaml" ]]; then
    cp examples/meluxina/config.yaml /etc/slurm/heartbeat/config.yaml
...
if [[ ! -f "/etc/slurm/heartbeat/server.crt" ]]; then
    ...
    openssl req -x509 -new -nodes -key /etc/slurm/heartbeat/ca.key \
        ...
        -subj "/C=LU/O=LuxProvide/OU=MeluXina/CN=meluxina-lux-lu"
