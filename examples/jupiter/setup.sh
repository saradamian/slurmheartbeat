#!/bin/bash
# Automated setup script for Slurm Heartbeat on JUPITER (FZ Jülich)
set -e

echo "=== Slurm Heartbeat Setup for JUPITER ==="
...
if [[ -f "examples/jupiter/config.yaml" ]]; then
    cp examples/jupiter/config.yaml /etc/slurm/heartbeat/config.yaml
...
if [[ ! -f "/etc/slurm/heartbeat/server.crt" ]]; then
    ...
    openssl req -x509 -new -nodes -key /etc/slurm/heartbeat/ca.key \
        ...
        -subj "/C=DE/O=FZJ/OU=Jupiter/CN=jupiter-fzj-de"
