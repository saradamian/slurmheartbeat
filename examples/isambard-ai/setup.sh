#!/bin/bash
# Automated setup script for Slurm Heartbeat on Isambard-AI (UK)
set -e

echo "=== Slurm Heartbeat Setup for Isambard-AI ==="
...
if [[ -f "examples/isambard-ai/config.yaml" ]]; then
...
if [[ ! -f "$HOME/.slurm/heartbeat/server.crt" ]]; then
    ...
    openssl req -x509 -new -nodes -key "$HOME/.slurm/heartbeat/ca.key" \
        ...
        -subj "/C=UK/O=ISAMBARD/CN=isambard-ai-uk"
