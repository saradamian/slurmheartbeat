#!/bin/bash
# Automated setup script for Slurm Heartbeat on ARRHENIUS (Sweden)
set -e

echo "=== Slurm Heartbeat Setup for ARRHENIUS ==="
...
if [[ -f "examples/arrhenius/config.yaml" ]]; then
...
if [[ ! -f "$HOME/.slurm/heartbeat/server.crt" ]]; then
    ...
    openssl req -x509 -new -nodes -key "$HOME/.slurm/heartbeat/ca.key" \
        ...
        -subj "/C=SE/O=ARRHENIUS/CN=arrhenius-se"
